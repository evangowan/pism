...
class SSARun(object):
    ...
    def solve(self):
        ...
        inputs.sea_level = 0
        inputs.surface_elevation = vecs.usurf
        inputs.ice_thickness = vecs.thk
        inputs.ice_enthalpy = vecs.enthalpy
        inputs.bed_elevation = vecs.topg
        inputs.basal_yield_stress = vecs.tauc
        if vecs.has('vel_bc'):
            inputs.bc_mask = vecs.bc_mask
            inputs.bc_values = vecs.vel_bc

        self.ssa.update(fast, inputs)
    ...
    def write(self, filename):
        ...
        ocean_pressure_difference = PISM.SSA_calving_front_pressure_difference(self.ssa).compute()
        vecs.add(ocean_pressure_difference)
...
class SSAFromInputFile(SSARun):
    ...
    def _initGrid(self):
        ...
        self.grid = PISM.IceGrid.FromFile(ctx, pio, "enthalpy", periodicity)
        pio.close()
