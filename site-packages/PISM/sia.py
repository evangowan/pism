...
def computeSIASurfaceVelocities(modeldata, siasolver=PISM.SIAFD):
    ...
    inputs.ice_thickness = md.vecs.thk
    inputs.bed_elevation = md.vecs.topg
    inputs.cell_type = md.vecs.mask
    inputs.ice_enthalpy = md.vecs.enthalpy
    inputs.surface_elevation = md.vecs.usurf

    sia.update(False, inputs, zero_sliding)
    u = sia.velocity_u()
    v = sia.velocity_v()
