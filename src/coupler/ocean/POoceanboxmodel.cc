  ...
  : PGivenClimate<POModifier, PISMOceanModel>(g, conf, NULL)
{
  ...
  if (ocean_oceanboxmodel_deltaT_set) {
    ...
    PISMOptionsString("-ocean_obm_deltaT",
    ...
    PISMOptionsReal("-ocean_obm_factor", "ocean_obm_factor set", delta_T_factor, delta_T_factor_set);
...
void BoxModel::init_impl() {
  ...
  basins = vars.get_2d_scalar("drainage_basins"); //if option drainageBasins set

  bool omeans_set;
  PISMOptionsIsSet("-ocean_means", omeans_set);
...
void BoxModel::define_variables(std::set<std::string> vars, const PIO &nc,
  ...
  PGivenClimate<POModifier, PISMOceanModel>::define_variables(vars, nc, nctype);
...
void BoxModel::initBasinsOptions(const POBMConstants &cc) {
  ...
  PISMOptionsInt("-number_of_basins", "Number of Drainage Basins", numberOfBasins, number_of_basins_set);
  ...
  PISMOptionsString("-ocean_means", "Input data name",
                             data_input, ocean_mean_set);
...
void BoxModel::update(double my_t, double my_dt) {
  ...
  basalmeltrate_shelf.copy_to(shelfbmassflux);//TODO Check if scaling with ice density
}
      ...
      if (roundbasins_set) {
        ...
        if( PetscAbs(id_fractional - static_cast<float>(id_rounded)) > 0.0) {
          ...
          if (id_fr_sw == static_cast<float>(id_ro_sw) && id_fr_sw != 0) {
            ...
            //if no neigbour has an integer id
            id = id_rounded;
          }
        } else {
...
void BoxModel::computeOCEANMEANS() {
  ...
  list.add(*salinity_ocean);  //salinitiy
  list.add(*basins);
  ...
  for(int k=0;k<numberOfBasins;k++) {
    ...
    if(k>0 && m_count[k]==0) {
      ...
      m_log->message(2, "PISM_WARNING: basin %d contains no ocean mean cells, no mean salinity or temperature values are computed! \n ", k);
    } else {
...
void BoxModel::extentOfIceShelves() {
  ...
  PISMGlobalSum(&lcounter_box_unidentified, &counter_box_unidentified, grid.com);
  for(int k=0;k<numberOfBasins;k++) {
    PISMGlobalSum(&lcounter[k], &counter[k], grid.com);
    PISMGlobalSum(&lcounter_CFbox[k], &counter_CFbox[k], grid.com);
    PISMGlobalSum(&lcounter_GLbox[k], &counter_GLbox[k], grid.com);
...
void BoxModel::identifyBOXMODELmask() {
  ...
  if(counter_box_unidentified>0.0) {
    //if there are still floating cells which were not labels before, i.e. a shelf connected to an island (in the ex_icerises case)
    ...
    for(int k=0;k<numberOfBasins;k++) {
      PISMGlobalSum(&lcounter_GLbox[k], &all_counter_GLbox[k], grid.com);
...
void BoxModel::extendGLBox() {
  ...
  PISMGlobalSum(&lcounter_box_unidentified, &all_counter_box_unidentified, grid.com);
...
void BoxModel::extendIFBox() {
  ...
  for(int k=0;k<numberOfBasins;k++) {
    PISMGlobalSum(&lcounter_CFbox[k], &all_counter_CFbox[k], grid.com);
...
void BoxModel::basalMeltRateForGroundingLineBox(const POBMConstants &cc) {
  ...
  for(int k=0;k<numberOfBasins;k++) {
    ...
    PISMGlobalSum(&lmean_meltrate_GLbox_vector[k], &mean_meltrate_GLbox_vector[k], grid.com);
    PISMGlobalSum(&lmean_salinity_GLbox_vector[k], &mean_salinity_GLbox_vector[k], grid.com);
    ...
    m_log->message(5, "  %d: cnt=%.0f, sal=%.3f, melt=%.3e, over=%.1e \n", k, counter_edge_of_GLbox_vector, mean_salinity_GLbox_vector[k], mean_meltrate_GLbox_vector[k], mean_overturning_GLbox_vector[k]) ;
  }
...
void BoxModel::basalMeltRateForIceFrontBox(const POBMConstants &cc) {
  ...
  m_log->message(4, "B2 : in bm ice front rountine\n");
      ...
      if (BOXMODELmask(i, j)==box_IF && shelf_id > 0.0) {
        ...
        if (area_GLbox==0) { // if there is no grounding line box in the current basin, set BOXMODELmask to 3 and compute basal melt rate by Beckmann-Gose
          //m_log->message(5, "!!!! GLBOX =0 , basin = %d at %d, %d, \n   ", shelf_id, i, j);
            ...
            //m_log->message(2, "!!!! ACF=%.3e, AGL=%.3e, MS=%.3f, MM=%.3f, MO=%.3f, basin = %d at %d, %d, \n   ", area_CFbox, area_GLbox, mean_salinity_in_GLbox, mean_meltrate_in_GLbox, mean_overturning_in_GLbox, shelf_id, i, j);
            //m_log->message(2, "!!!! k1=%.3f, k2=%.3f, k3=%.3f, k4=%.3f, k5=%.3f, k6=%.3f, basin = %d at %d, %d, \n   ", k1, k2, k3, k4, k5, k6, shelf_id, i, j);

            //! salinity for calving front box
            if (k4 == 0.0) {
              //m_log->message(5, "PISM_ERROR: Division by zero! k4=%f at %d, %d\n   Aborting... \n", k4, i, j);
              //m_log->message(5, "PISM_ERROR: Probably mean_overturning_in_GLbox = %f is zero, check if there is a grounding line box in basin %d , \n   ", mean_overturning_in_GLbox, shelf_id);
            ...
            if ((0.25*PetscSqr(k5/k4) - (k6/k4)) < 0.0) {
              //m_log->message(5, "PISM_ERROR: Square-root is negative! %f at %d, %d\n...with 0.25*PetscSqr((k5/k4))=%f, (k6/k4)=%f, k5=%f, k6=%f, k4=%f\n   Aborting... \n", 0.25*PetscSqr(k5/k4) - (k6/k4), i, j, 0.25*PetscSqr((k5/k4)), (k6/k4), k5, k6, k4) ;
              //PISMEnd();
              lcountSqr+=1;
              BOXMODELmask(i, j) = box_other;
              continue;
